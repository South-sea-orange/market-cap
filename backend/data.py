...
_EMPTY = {"groups": {"ALL": [], "US": [], "KR": []}, "history": {},
          "fx_krw_per_usd": 1513, "as_of": "", "updated_at": "", "source": "companiesmarketcap.com"}
KST = datetime.timezone(datetime.timedelta(hours=9))


def _with_update_meta(payload: dict, mtime: float) -> dict:
    if not payload.get("updated_at"):
        payload["updated_at"] = datetime.datetime.fromtimestamp(mtime, KST).strftime("%Y-%m-%d %H:%M")
    if not payload.get("as_of") and payload.get("updated_at"):
        payload["as_of"] = payload["updated_at"][:10]
    return payload
    ...
    with _lock:
        ...
        if _cache["data"] is None or m != _cache["mtime"]:
            with open(DATA_FILE, encoding="utf-8") as f:
                _cache["data"] = _with_update_meta(json.load(f), m)
            _cache["mtime"] = m
        return _cache["data"]
